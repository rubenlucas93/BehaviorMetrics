...
class Brain:
    ...
    def __init__(self, sensors, actuators, handler, config=None):
        ...
        args = {
            ...
            'environment': 'simple',
            'agent': 'f1',
            'filename': 'brains/CARLA/config/config_inference_followlane_sb_ddpg_2_f1_carla.yaml'
        }
        ...
        self.x_row = [333, 345, 425, 475, 525]
        params = InferenceExecutorValidator(**inference_params)
        inference_file_w = params.inference["params"]["retrain_ddpg_tf_model_name_w"]
        inference_file_v = params.inference["params"]["retrain_ddpg_tf_model_name_v"]

        self.w_net = DDPG.load(inference_file_w)
        self.v_net = DDPG.load(inference_file_v)
    ...
    def get_inference(self, config_file: dict, input_inference: str) -> dict:
        return {
            ...
            "params": config_file["retraining"][input_inference],
        }
    ...
    def execute(self):
        ...
        curve_indicator = max(-1, 1-(abs(state[0])*5))
        state.append(speed/40)
        state.append(w_angle)
        state.append(curve_indicator)

       # print(f"speed {speed/40}")
       # print(f"angle {w_angle}")
       # print(f"crv_ind {curve_indicator}")
       # print("________________")

        # if not bad_perception and abs(average_difference) < 0.07:
        # if not missing_line:
        [action_w, _] = self.w_net.predict(np.array(state), deterministic=False)
        # state[5] = speed/40 * 1.25
        [action_v, _] = self.v_net.predict(np.array(state), deterministic=False)
        ...
        action = [action_v[0], action_w[1] * 0.2]
