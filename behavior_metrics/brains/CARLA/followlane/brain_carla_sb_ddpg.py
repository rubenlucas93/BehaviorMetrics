...
class Brain:
    ...
    def __init__(self, sensors, actuators, handler, config=None):
        ...
        if config and 'filename' in config:
            ...
            filename = 'brains/CARLA/followlane/config/config_inference_followlane_sb_ddpg_f1_carla.yaml'
    ...
    def execute(self):
        ...
        dists = np.mean(x_normalized[:5])  # Take 7 elements, apply abs, then mean
        # dists = np.mean(state)
        # print(dists)
        # print(state[:10])
