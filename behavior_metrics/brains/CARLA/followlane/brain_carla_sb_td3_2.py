...
class Brain:
    ...
    def __init__(self, sensors, actuators, handler, config=None):
        ...
        self.tensorboard = ModifiedTensorBoard(
            log_dir=f"logs/Tensorboard/td3/{time.strftime('%Y%m%d-%H%M%S')}"
        )
        ...
        args = {
            ...
            'environment': 'simple',
            'agent': 'f1',
            'filename': 'brains/CARLA/followlane/config/config_inference_followlane_sb_td3_f1_carla_2.yaml'
        }
        ...
        self.td3_agent = TD3.load(inference_file)
    ...
    def execute(self):
        ...
        [action, _] = self.td3_agent.predict(np.array(self.previous_states), deterministic=True)
