...
def run_yolop_v2_inference(image, model, transform, device):
    ...
    return da_seg_out, ll_seg_out, image
...
def detect_lanes_yolop_v2_hybrid_agent(image, model, transform, device, save_path_intermediate_dir=None, img_path=None, force_drivable_fallback=False, force_classical_fallback=False, reference_point=None):
    # Step 1 & 2: Calculate both drivable area and lines first
    with torch.no_grad():
        da_seg_out, ll_seg_out, resized_image = run_yolop_v2_inference(image, model, transform, device)
    ...
    if len(path_points_for_sampling) > 10:
        try:
            # Sort points by y-coordinate to ensure correct interpolation
            path_points_for_sampling = path_points_for_sampling[path_points_for_sampling[:, 1].argsort()]
    ...
    return (final_mask / 255).astype(np.uint8), distance_to_center, center_lanes, center_lanes, cv2.cvtColor(ll_segment, cv2.COLOR_GRAY2BGR), all_extended_lines_mask_from_tracker, all_paths_viz_from_tracker, lines_debug_viz_from_tracker
...
class LaneDetector:
    ...
    def process_image(self, image):
        ...
        final_mask, distance_to_center, center_lanes, _, raw_detection_image, extended_image, paths_image, points_for_extension = detect_lanes_yolop_v2_hybrid_agent(
            resized_image, self.yolop_v2_lines_model, self.yolop_v2_lines_transform, self.device, img_path=None
        )
